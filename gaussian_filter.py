...
def plot_images():
    ...
    plt.title(f'Gaussian Filtered Image (Kernel Size = {kernel_size})')
    plt.axis('off')
...
def plot_canny():    
    ...
    plt.subplot(2, 3, 3)
    plt.imshow(filtered_image, cmap='gray')
    plt.title(f'{noise_name} Filtered Image (Kernel Size = {kernel_size})')
    plt.axis('off')
    plt.subplot(2, 3, 6)
...
filtered_image, filter_time = apply_gaussian_filter(noisy_image, kernel_size)
